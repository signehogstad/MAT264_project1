function p = momentum_ssb(planet_name, position_vectors, masses, dir)
...
while i < (k+1)
    ...
    for j = 1:3
        contr = masses(i)*(pos(j) - position_vectors(j))*((pos(1)-position_vectors(1))^2+(pos(2)-position_vectors(2))^2+(pos(3)-position_vectors(3))^2)^(-3/2);
